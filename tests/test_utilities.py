...
@parametrize('case', [
    dict(funcs=(lambda x: '!!!' + x + '!!!', lambda x: 'Hi {0}'.format(x)),
         args=('Bob',),
         expected='Hi !!!Bob!!!'),
...
@parametrize('case', [
    dict(args=(lambda a, b: a + b, 1, 2),
         expected=3),
    dict(args=(None, 1, 2),
         expected=1),
    dict(args=({'a': 1, 'b': 2}, {'a': 1, 'b': 2, 'c': 3}),
         expected=True),
    dict(args=({'a': 1, 'b': 2}, {'a': 4, 'b': 2, 'c': 3}),
         expected=False),
    dict(args=({'a', 'b'}, {'a': 1, 'b': 2, 'c': 3, 'd': 4}),
         expected={'a': 1, 'b': 2}),
    dict(args=(('a', 'b'), {'a': 1, 'b': 2, 'c': 3, 'd': 4}),
         expected=(1, 2)),
    dict(args=('a', {'a': 1, 'b': 2}),
...
@parametrize('case', [
    ...
    dict(args=(25,),
         expected={'type': int, 'min': 0, 'max': 25}),
    dict(args=(5, 10),
         expected={'type': int, 'min': 5, 'max': 10}),
    dict(args=(), kwargs={'floating': True},
         expected={'type': float, 'min': 0, 'max': 1}),
    dict(args=(25,), kwargs={'floating': True},
         expected={'type': float, 'min': 0, 'max': 25}),
    dict(args=(5, 10), kwargs={'floating': True},
    ...
    dict(args=(5.0, 10.0),
         expected={'type': float, 'min': 5, 'max': 10}),
    dict(args=(5.0, 10.0), kwargs={'floating': True},
         expected={'type': float, 'min': 5, 'max': 10}),
])
...
@parametrize('case', [
    ...
    dict(args={'attempts': 1},
         expected={'count': 0, 'times': []}),
    dict(args={'attempts': 3, 'delay': 0.5, 'scale': 2.0},
         expected={'count': 2, 'times': [0.5, 1.0]}),
    dict(args={'attempts': 5, 'delay': 1.5, 'scale': 2.5},
         expected={'count': 4, 'times': [1.5, 3.75, 9.375, 23.4375]}),
    dict(args={'attempts': 5, 'delay': 1.5, 'max_delay': 8.0, 'scale': 2.5},
...
@parametrize('case', [
    dict(args={'jitter': 5, 'delay': 2, 'scale': 1, 'attempts': 5},
         unexpected=[2, 2, 2, 2]),
    dict(args={'jitter': 10, 'delay': 3, 'scale': 1.5, 'attempts': 5},
         unexpected=[3, 4.5, 6.75, 10.125]),
    dict(args={'jitter': 1.0, 'delay': 3, 'scale': 1.5, 'attempts': 5},
         unexpected=[3, 4.5, 6.75, 10.125]),
])
...
@parametrize('case', [
    dict(args={'attempts': 1, 'exceptions': (RuntimeError,)},
         expected={'exception': RuntimeError, 'count': 0}),
    dict(args={'attempts': 2, 'exceptions': (RuntimeError,)},
